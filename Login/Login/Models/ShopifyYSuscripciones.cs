...
namespace Login.Models
{
    public class ShopifyYSuscripciones
    {
        ...
        public List<GRAFICO> informe_interactivo { get; set; }
        public List<GRAFICO> reporte_360 { get; set; }
        ...
        public ShopifyYSuscripciones(List<JToken> jtoken)
        {
            ...
            this.informe_interactivo = new List<GRAFICO>();
            this.reporte_360 = new List<GRAFICO>();
            ...
            foreach (var item in entrada)
            {
                ...
                else
                {
                    ...
                    switch (item.SKU.Split('-')[0])
                    {
                        ...
                        case "datainformeinteractivo":
                            ...
                            this.informe_interactivo.Add(dbGrafico.GRAFICO.Where(x => x.id == idGenerico).First());                       
                            break;
                        case "datareporte360":
                            //datareporte360-1234
                            idGenerico = Int32.Parse(item.SKU.Split('-')[1]);
                            this.reporte_360.Add(dbGrafico.GRAFICO.Where(x => x.id == idGenerico).First());                                                    
                            break;
                        default:
                            //this.producto_Shopifies.Add(item);
                            break;
                ...
                {                 
                    ...
                    case "datainformeinteractivo":
                        ...
                        //idGenerico = int.Parse(sku[1]);
                        //this.informe_interactivo.Add(dbGrafico.GRAFICO.Where(x => x.id == aux2).First());
                        //this.informe_interactivo.Add(dbGrafico.GRAFICO.Where(x => x.id == idGenerico).First());                       
                        break;
                    case "datareporte360":
                        ayuda = sku[1];
            ...
            foreach (var item in sector)
            {
                ...
                var grafico = dbGrafico.INDUSTRIA.Where(x => x.id == 10 ).First();
                indAux.Add(grafico);
            }
